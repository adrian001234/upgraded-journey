...
New design: this script does ONE unit of work per invocation:
  ...
  5. If that was the last shot, flip generation_status to 'assembling'.
...
def render_shot_via_agnes(scene_description):
    """Call Agnes AI to render a single ~8-9 second clip for this shot."""
    body = json.dumps({
...
def bump_shots_completed(pipeline_id, new_count, total_shots):
    ...
    if new_count >= total_shots:
        update["generation_status"] = "assembling"
...
def main():
    ...
    if not shot:
        # All shots already generated for this row but status wasn't flipped yet - fix it and exit.
        print(f"Pipeline {pipeline_id}: no pending shots left, flipping to assembling.")
    ...
    try:
        ...
        # Reset to pending so the next tick retries it rather than skipping it permanently.
        time.sleep(1)
        _supabase_request("PATCH", f"video_shots?id=eq.{shot['id']}", {"status": "pending"})
